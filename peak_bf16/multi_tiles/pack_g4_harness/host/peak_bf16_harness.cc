...
std::vector<test_harness::test_harness_args> make_args(
    std::array<std::vector<std::uint16_t>, NumAInputs>& input_a,
    std::array<std::vector<std::uint16_t>, NumBInputs>& input_b,
    std::vector<std::uint16_t>& output,
    int repetitions,
    int channel_delay) {
    std::vector<test_harness::test_harness_args> args;
    for (int i = 0; i < NumAInputs; ++i) {
        args.push_back({static_cast<test_harness::channel_index>(
                            test_harness::PLIO_01_TO_AIE + i),
                        AInputBytes,
                        static_cast<std::size_t>(repetitions),
                        static_cast<std::uint64_t>(channel_delay),
                        reinterpret_cast<char*>(input_a[i].data())});
    }
    for (int i = 0; i < NumBInputs; ++i) {
        args.push_back({static_cast<test_harness::channel_index>(
                            test_harness::PLIO_01_TO_AIE + NumAInputs + i),
                        BInputBytes,
                        static_cast<std::size_t>(repetitions),
                        static_cast<std::uint64_t>(channel_delay),
                        reinterpret_cast<char*>(input_b[i].data())});
    }
...
void run_harness_bf16(std::uint16_t* output,
    ...
    for (int i = 0; i < NumAInputs; ++i) {
        if (input_a[i] == nullptr) {
            throw std::invalid_argument("run_harness_bf16 received null A pointer.");
        }
    }
    for (int i = 0; i < NumBInputs; ++i) {
        if (input_b[i] == nullptr) {
            throw std::invalid_argument("run_harness_bf16 received null B pointer.");
        }
    }
    ...
    std::array<std::vector<std::uint16_t>, NumAInputs> padded_input_a;
    std::array<std::vector<std::uint16_t>, NumBInputs> padded_input_b;
    std::vector<std::uint16_t> padded_output(CElements * graph_iterations);

    for (int ch = 0; ch < NumAInputs; ++ch) {
        padded_input_a[ch].resize(AElements * graph_iterations);
    }
    for (int ch = 0; ch < NumBInputs; ++ch) {
        padded_input_b[ch].resize(BElements * graph_iterations);
    }

    for (std::size_t i = 0; i < graph_iterations; ++i) {
        for (int ch = 0; ch < NumAInputs; ++ch) {
            std::copy_n(input_a[ch],
                        AElements,
                        padded_input_a[ch].data() + i * AElements);
        }
        for (int ch = 0; ch < NumBInputs; ++ch) {
            std::copy_n(input_b[ch],
                        BElements,
                        padded_input_b[ch].data() + i * BElements);
        }
    }

    std::cout << "Using XCLBIN file: " << XclbinPath << '\n';
    std::cout << "Running peak BF16 G4 pack through Test Harness\n";
    std::cout << " - Graph                         : " << GraphName << '\n';
    std::cout << " - Pack Y/G/X                    : "
              << PackY << "/" << PackG << "/" << PackX << '\n';
    std::cout << " - AIE kernels                   : " << NumKernels << '\n';
    std::cout << " - A input PLIOs                 : " << NumAInputs << '\n';
    std::cout << " - B input PLIOs                 : " << NumBInputs << '\n';
    std::cout << " - C output PLIOs                : " << NumCOutputs << '\n';
    std::cout << " - Input GEMM shape              : "
              << InputM << "x" << InputK << "x" << InputN << '\n';
    std::cout << " - A input bytes/PLIO/iteration  : " << AInputBytes << '\n';
    std::cout << " - B input bytes/PLIO/iteration  : " << BInputBytes << '\n';
